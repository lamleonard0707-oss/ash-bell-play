...
function figure(c,x,y,id,t=0,scale=1,dx=1,dy=1,action=0){
 ...
 const stepping=p&&id===chosen&&heroMotion?Math.abs(Math.sin(walkPhase)):0,lift=stepping*3.2*scale,lean=p&&id===chosen&&heroMotion?Math.sin(walkPhase*.5)*.03*(f.flip?-1:1):0;
 c.save();c.fillStyle='#080503';c.globalAlpha=.44*(1-lift/9);c.beginPath();c.ellipse(x,y+1,18*scale*(1-lift/34),7*scale*(1-lift/22),0,0,7);c.fill();c.globalAlpha=1;
...
function hurt(d,ailment,opts){const dot=!!(opts&&opts.dot);if(mode!=='play')return;if(!dot&&p.inv>0)return;if(ailment==='frost')p.chill=2.5;if(ailment==='venom'){p.poison=4;p.poisonTick=1}d*=difficulty().damage;d*=(equipped.amulet?.effect==='guard'?.88:1)*(1-Math.min(60,gearStats().armor+attrArmor())/100);const absorb=Math.min(p.shield||0,d);p.shield=Math.max(0,(p.shield||0)-absorb);p.hp-=d-absorb;if(!dot)p.inv=.32;damageFlash=dot?.28:.55;shake=dot?2:6;ring(p.x,p.y,dot?'#9ac25a':'#dc6554',dot?20:30,.2);if(!dot)playSfx('impact',p.x,1);if(p.hp<=0){p.hp=0;end(false)}}
...
function updateHUD(){if(!p)return;
 ...
 expansionHUD();journeyHUD();monkHUD();echoHUD();exploreHUD();const boss=foes.find(f=>f.type===3);if(boss)$('#bossbar i').style.width=Math.max(0,boss.hp/boss.maxhp*100)+'%';
}
function update(dt){
 if(impactFreeze>0){impactFreeze-=dt;return}elapsed+=dt;attackCD-=dt;skillCD=Math.max(0,skillCD-dt);dashCD=Math.max(0,dashCD-dt);p.inv-=dt;p.boost-=dt;p.action=Math.max(0,p.action-dt);p.mp=Math.min(100,p.mp+((equipped.amulet?.effect==='mana'?7:5)+gearStats().regen+buildManaRegen()+attrManaRegen())*dt*(chosen===5?.22:1));
 ...
 if(len){dx/=Math.max(1,len);dy/=Math.max(1,len);p.dx=dx;p.dy=dy;const speed=classes[chosen].speed*movementFactor()*(1+(speedMult-1)*.53)*(1+(gearStats().speed+attrSpeed())/100)*(p.boost>0?1.6:1)*(p.chill>0?.65:1);p.x+=dx*speed*dt;p.y+=dy*speed*dt;confine(p)}
 updateTravel(moveStart,dt);updateSkillVisuals(dt);updateExpansion(dt);updateEcho(dt);if(mode!=='play')return;if(attackHeld||keys.j)attack();
 ...
 for(const d of drops){d.life-=dt;
  if(d.type==='gear'){d.retry=Math.max(0,(d.retry||0)-dt);if(dist(d,p)<36&&d.retry<=0){if(awardGear(d.item))d.life=0;else d.retry=3}continue}
...
function render(){
 ...
 if(!selecting){
  ...
  for(const f of entities){if(f.x<cam.x-100||f.x>cam.x+visibleW+100||f.y<cam.y-50||f.y>cam.y+visibleH+185)continue;if(f.player){
   ...
   ctx.globalAlpha=p.inv>0?.7+Math.sin(time*38)*.2:1;figure(ctx,p.x,p.y,chosen,time,1,p.dx,p.dy,p.action);ctx.globalAlpha=1;
...
initFeatures();initExpansion();initMouse();initCharacterUI();initEcho();initCampaign();initTerrainArt();initJourney();initMotion();initBuildcraft();initDirectional();initCinematics();
requestAnimationFrame(frame);
