...
function resetExpansion(){ranks=[1,0,0];resetProgression();skillPoints=2;activeSkill=0;skillTimers=[0,0,0];castFields=[];wards=[];spawnSerial=0;routeStage=0;p.shield=0;p.chill=0;p.poison=0;p.poisonTick=0;p.satiety=0;p.curse=0;p.rewind=null;echoQueue=[];selectedTreeNode={branch:0,tier:0};$('#treebtn').hidden=false;seedEquipment();syncSkillLabel()}
...
function releaseExpansion(c){const b=c.branch||0,r=c.rank||1;castingBranch=b;const power=gearPower()*(1+(r-1)*.3)*skillPowerMult(b);playSfx('cast',c.x,c.type);shake=5;if(c.type===4){releaseMonk(c);return}if(c.type===5){releaseEcho(c);return}
