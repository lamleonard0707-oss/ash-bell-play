...
function defaultControl(id){const landscape=W>H&&H<600,mobile=W<600;const map=landscape?{joystick:[85,H-95],potion:[W-194,H-100],dash:[W-194,H-49],skill:[W-135,H-76],attack:[W-60,H-61]}:mobile?{joystick:[73,H-189],potion:[W-173,H-201],dash:[W-173,H-148],skill:[W-116,H-182],attack:[W-49,H-160]}:{joystick:[106,H-191],potion:[W-234,H-227],dash:[W-234,H-162],skill:[W-162,H-198],attack:[W-77,H-176]};return map[id]}
function positionControl(id,x,y){const el=$('#'+id),scale=preferences.size/100,w=el.offsetWidth||({joystick:100,attack:68,skill:60,dash:46,potion:46}[id]),h=el.offsetHeight||w;const margin=w*scale/2+8,topLimit=H<600&&W>H?70:105,bottomLimit=H<600&&W>H?10:92;x=Math.max(margin,Math.min(W-margin,x));y=Math.max(topLimit+h*scale/2,Math.min(H-bottomLimit-h*scale/2,y));el.style.left=(x-w/2)+'px';el.style.top=(y-h/2)+'px';el.style.bottom='auto';el.style.right='auto';el.style.margin='0';el.style.transform='scale('+scale+')';el.style.opacity=preferences.opacity/100;return {x,y}}
